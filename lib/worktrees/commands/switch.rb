  ...
  module Commands
    class Switch < Dry::CLI::Command
      ...
      def call(name:, **options)
        ...
        raise ValidationError, "Worktree '#{name}' not found" unless target_worktree

        # Check current state for warnings/blocking
        current = manager.current_worktree
        if current&.dirty? && !options[:force]
        ...
        puts "\nWarning: Previous worktree '#{current.name}' has uncommitted changes" if current&.dirty?
